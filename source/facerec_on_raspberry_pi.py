...
# Store encoding and person's name into image_encodings array
for photo in photos:
    underscore = photo.index('_')
    dot = photo.index('.')
    first = photo[:underscore]
    last = photo[underscore+1:dot]
    image = face_recognition.load_image_file(photo)
    encoding = face_recognition.face_encodings(image)[0]
    image_encodings.append((encoding, first + " " + last))
